...
export default function Modal ({ children, title = null, isModalOpen = false, setIsModalOpen }) {
  const { primaryColor } = useSelector((state) => state.theme)

  return isModalOpen && <BGModal>
    <ModalContainer>
      { title && <ModalHead>
        <FlexBetween>
          <Text clave={title} color={primaryColor} type="title" />
          <button style={{ backgroundColor: 'transparent', border: '0px', cursor: 'pointer' }} onClick={() => setIsModalOpen(false)}>
